...
function codeGenAndDiff()
{
  ...
  if [ "${SCRBCK}" == "gridpack" ] && [ "${UNTARONLY}" == "1" ]; then
    ...
    echo "${cmd}" >> ${outproc}.mg
    if [ "${OUTBCK}" == "mad" ]; then # $SCRBCK=sycl and $OUTBCK=mad
  ...
  # Output directories: examples ee_mumu.sa for sycl
  autosuffix=sa
  if [ "${OUTBCK}" == "mad" ]; then
    autosuffix=${OUTBCK}
  fi
  ...
  # Fix build errors which arise because the autogenerated directories are not relocatable
  if [ "${OUTBCK}" == "mad" ]; then
  ...
  # Use strategy SDE=1 in multichannel mode
  if [ "${OUTBCK}" == "mad" ]; then
...
function usage()
{
  # NB: all options with $SCRBCK=sycl use the gpu branch
  echo "Usage: $0 [--nobrief] [--mad] [--nopatch|--upstream] <proc>"
  exit 1
}
...
OUTDIR=$(dirname $SCRDIR)/procs # e.g. mg4gpu-sycl/procs if $SCRDIR=mg4gpu-sycl/src
mkdir -p ${OUTDIR}

# Script directory backend (sycl, gridpack or alpaka)
SCRBCK=sycl

# Default output backend (in the sycl directory this can be changed using commad line options like --mad)
OUTBCK=$SCRBCK
...
# Check that MG5aMC uses the gpu branch (default for sycl)
...
if [ "${OUTBCK}" == "no-path-to-this-statement" ]; then
  echo -e "\nWARNING! '${OUTBCK}' mode selected: do not copy the sycl plugin"
else
